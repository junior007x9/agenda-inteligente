          ...
          <div className="min-w-[700px] lg:min-w-full">
            ...
            <div className="grid grid-cols-7 gap-2 md:gap-3 auto-rows-fr">
              {days.map((slot, index) => {
                ...
                return (
                  <div key={slot.date} className={`min-h-[110px] md:min-h-[130px] p-2 md:p-3 rounded-2xl border transition-all flex flex-col ${isToday ? 'bg-indigo-900/20 border-indigo-500/50 shadow-inner' : 'bg-slate-900/60 border-slate-800/50 hover:border-slate-600'}`}>
                    <div className={`text-xs font-bold w-7 h-7 flex items-center justify-center rounded-full mb-2 ${isToday ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400'}`}>
                      {slot.date}
                    </div>
                    
                    <div className="space-y-1.5 flex-1">
                      {dayMeetings.map(m => (
                        <div key={m.id} className="text-[10px] bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-1 rounded-md line-clamp-2 leading-tight">
                          <span className="font-bold">{new Date(m.startTime).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</span> <br/>{m.title}
                        </div>
                      ))}
                      {dayTasks.map(t => (
                        <div key={t.id} className={`text-[10px] px-2 py-1 rounded-md line-clamp-1 border ${t.status === 'concluido' ? 'bg-slate-800 text-slate-500 border-slate-700 line-through' : 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30'}`}>
                          {t.title}
                        </div>
                      ))}
                    </div>
